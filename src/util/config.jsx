...
http.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      //Chuyển hướng trang mà không cần reload lại trang để giữ đc các state hiện tại trên redux
      history.push("/login");
    }
...
/* Các status code thường gặp
  ...
  201 : Request gửi đi thành công và đã đc khởi tạo
  400 : bad request => request gửi đi thành công tuy nhiên không tìm thấy dữ liệu từ tham số gửi đi
  404 : Not found (không tìm thấy api đó) , hoặc tương tữ 400

  401 : Unauthorize token không hợp lệ không có quyền truy cập vào api đó

  403 : Forbinden token hợp lệ tuy nhiên chưa đủ quyèn truy cập vào api đó 

  500 : Error server (Lỗi xảy ra trên server có khả năng là fontend gửi dữ liệu chưa hợp lệ dẫn đến backend xử lí bị lỗi )
 */
